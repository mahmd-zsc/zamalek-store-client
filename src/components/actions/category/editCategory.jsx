...
function EditCategory() {
  ...
  // Fetching category and handling loading/error states
  ...
  const handleFileInputChange = async (event) => {
    ...
    await dispatch(updateCategoryImage(category._id, formData));
  ...
  // Populate form data on category change
  useEffect(() => {
    if (category) {
      setFormData({
        name: category.name,
  ...
  return (
    <div className="roboto-medium capitalize rounded-lg px-2 pb-4">
      <div className="flex justify-between items-center pb-4">
        <h5 className="text-2xl font-bold">edit category</h5>
        <FontAwesomeIcon
          icon={faTimes}
          size="lg"
